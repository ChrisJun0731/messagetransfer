...
public class MyUtil {
     ...
     * @return 高低位存储的字节数组
     */
    public byte[] convertInt2Byte(int num) {
        byte lower = (byte) (num % 256);
        byte higher = (byte) (num / 256);
        byte[] result = new byte[]{lower, higher};
        return result;
    }
     ...
     * @return
     */
    public int conventByte2Int(byte low, byte high){
        int lower = (low+256)%256;
        int higher = (high+256)%256;
     ...
     * @return 校验码
     */
    public byte[] Pub_CalcCRC(byte[] dataFrame){
        int fcs = 0xffff;
        for(int i=0; i<dataFrame.length; i++){
            fcs = ((fcs>>8)^fcstab[(fcs^dataFrame[i])&0xff]);
        }
        return convertInt2Byte(fcs);
    }
